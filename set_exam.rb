...
class Adel_exam
	...
	def initialize
		...
		#qs = CGI.new
    		#@params = qs.params
    		@params = Hash.new
		#@params = {"mode" => nil} # テスト用 #"mode" => "pre_evaluate", 
		@params = {"selected" => "q1_4_1", "type" => "radio", "value" => "1"} # テスト用
		# REXML::Document
		@doc = nil
	end
...
case m_test.get_mode(adel.get_params)
	when "set" then # 最初の呼び出し
		...
		m_test.get_xhtmlAll(adel.create_doc())
